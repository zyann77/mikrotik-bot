...
// Token Bot dari @BotFather (Gunakan token baru yang sudah kamu revoking)
const bot = new TelegramBot('8588037946:AAGX3v2A855p676F1nB676-XN_OcT_XXXXX', { polling: true }); 

// ID Telegram Kamu (Bos)
const ID_TELEGRAM_SAYA = 7917320065; 
const sesiTeknisi = {};

console.log('Bot RnBNET (COMPATIBILITY MODE) Berjalan...');
...
bot.on('message', async (msg) => {
    ...
    if (dataSesi && dataSesi.status === 'WAITING_FOR_NAME') {
        ...
        const api = new RouterOSClient({
            ...
            password: pass,
            port: port,
            timeout: 8, // Tambah waktu sedikit
            keepalive: false
        });
        ...
        const koneksiMaksimal = setTimeout(async () => {
            if (isDone) return;
            isDone = true;
            await bot.editMessageText(`❌ Gagal terhubung ke Server ${serverLabel}: Batas waktu habis.`, { chat_id: chatId, message_id: infoMsg.message_id });
        ...
        try {
            ...
            // PERBAIKAN SINTAKS: Menggunakan format query yang lebih kompatibel dengan v6
            const userQuery = await api.write([
                '/ppp/secret/print',
                '?.pro=name',  // Filter berdasarkan properti name
                `?=${username}` // Isinya adalah username
            ]);
            ...
            // Cek Status Aktif
            const activeQuery = await api.write([
                '/ppp/active/print',
                '?.pro=name',
                `?=${username}`
            ]);
            ...
            const lastLogout = userObj['last-logged-out'] || 'Belum pernah login';
            ...
            const waktuSederhana = new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: 'Asia/Jakarta' }) + ' WIB';

            const teksSukses = `✨ *RnB Network System Interface* ⚡️\n` +
                               `-----------------------------------------------\n` +
                               `📝 *Status Aktif :* SUKSES ✅\n` +
                               `👤 *Pelanggan :* ${username}\n` +
            ...
            if (!isDone) {
                isDone = true;
                clearTimeout(koneksiMaksimal);
                bot.editMessageText(teksSukses, { chat_id: chatId, message_id: infoMsg.message_id, parse_mode: 'Markdown' });
                bot.sendMessage(ID_TELEGRAM_SAYA, `📢 *LAPORAN AKTIVASI*\n👷 *Eksekutor :* ${namaTeknisi}\n👤 *User :* ${username}\n📍 *Server :* ${serverLabel}`, { parse_mode: 'Markdown' });
            }
